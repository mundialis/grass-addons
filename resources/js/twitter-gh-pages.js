(function ($, undefined) {
    ...
    function addRepos(repos, page) {
      ...
      https://api.github.com/search/repositories?q=topic:grass-gis-addons+org:mundialis

      var uri = "https://api.github.com/search/repositories?callback=?"
              + "&q=topic:grass-gis-addons+org:mundialis"
              + "&per_page=100"
              + "&page="+page;

      $.getJSON(uri, function (result) {
        if (result.data && result.data.items && result.data.items.length > 0) {
          repos = repos.concat(result.data.items);
    ...
    function getNumMembers(page, numMembers) {
      ...
      var membersUri = "https://api.github.com/orgs/mundialis/members?callback=?"
        + "&per_page=100"
        + "&page="+page;
    ...
    function makeLarry(sizeName, speed) {
      ...
      var $img = $("<img>")
        ...
        .attr("src", "resources/img/grasslogo.svg")
        .attr("width", size)
        .attr("height", size)
        .css({
          position: "absolute",
